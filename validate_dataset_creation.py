...
def find_video_file(base_dir, link_id):
    ...
    pattern = re.compile(link_id+'.mp4', re.IGNORECASE)
...
def parse_json_to_video_info(json_path, video_base_dir):
    ...
    for entry in data:
        ...
        if not video_path:
            print(f"⚠ Skipping {link_id} - video file not found")
            continue
...
if __name__ == "__main__":
    ...
    JSON_PATH = "MSASL_transformed.json"
    VIDEO_BASE_DIR = "Asl_videos/asl_train_videos"
    OUTPUT_DIR = "asl_landmarks_output/train_dataset"
